...
function checkPoolsPrices(pools: PoolData[], prices: Record<string, number>) {
  ...
  if (likelyHaveError.length > 0) {
    console.warn(
      `[WARN] ${likelyHaveError.length} AMM pools likely have bad prices/liquidity which could have knock-on effects.`
    );
...
async function fetchChainPools(chain: ChainId, pools: Pool[]): Promise<PoolData[]> {
  ...
  const results = await batchMapRetry<Pool, PoolData>({
    ...
    handleFn: async batch => {
      const results = await multicallContract.read.getLpInfo([
        batch.map(p => [p.address, p.lp0.address, p.lp1.address]),
      ]);
      return batch.map((pool, i) => ({
        ...pool,
        totalSupply: new BigNumber(results[i * 3 + 0].toString(10)),
        lp0: {
          ...pool.lp0,
  ...
  const failed = results.filter(isContextResultRejected).map(r => r.context.name);
  if (failed.length > 0) {
    console.warn(`Failed to fetch data for ${failed.length} pools on chain ${chain}`, failed);
  }

  // TODO decide what to do in this situation
  // if one or more batches fail, then some pools will not have totalSupply/balance
  // previously if any batch failed, all values would be BigNumber(undefined) [NaN]
  // in this version, any pools from those chunks are not returned
  // thus, will not be used for price calcs, or have lp price/breakdown available
