import { LpBreakdownData, LpBreakdowns, LpPool, LpPoolWithBalance } from '../types/LpPool';
...
import MulticallAbi from '../abis/BeefyPriceMulticall.json';

const MULTICALLS: Record<ChainId, string> = {
...
function sortByKeys<TKey extends string, TValue, TType extends Record<TKey, TValue>>(
  o: TType
): TType {
  return Object.keys(o)
    .sort()
    .reduce((r, k) => ((r[k] = o[k]), r), {} as TType);
}
...
const calcLpPrice = (pool: LpPoolWithBalance, tokenPrices: TokenPrices): LpBreakdownData => {
...
export async function fetchAmmPrices(pools: LpPoolWithBalance[], knownPrices: TokenPrices) {
  let prices: TokenPrices = { ...knownPrices };
  let lps: TokenPrices = {};
  let breakdown: LpBreakdowns = {};
  let weights = {};
