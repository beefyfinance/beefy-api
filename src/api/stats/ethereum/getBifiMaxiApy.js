...
const getYearlyRewardsInUsd = async () => {
  ...
  for (let i = 0; i < rewards.length; ++i) {
    ...
    if (periodFinish.isGreaterThan(Math.floor(Date.now() / 1000))) {
      yearlyRewards = yearlyRewards.plus(rewardRate.times(secondsPerYear).times(rewardPrice).dividedBy(rewards[i].decimals));
    }
  };
...
const getTotalStakedInUsd = async () => {
  ...
  const totalStaked = new BigNumber(
    (await tokenContract.read.balanceOf([rewardPool])).toString()
  );
