...
export async function getYieldBasisPrices(tokenPrices) {
  let prices = {};

  const gaugeToYbCalls = pools.map(p => fetchContract(p.gauge, abi, chainId).read.previewRedeem([1e18]));
  const ybToAssetCalls = pools.map(p => fetchContract(p.address, abi, chainId).read.preview_withdraw([1e18]));
