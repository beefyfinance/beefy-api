...
import type { EulerPool } from './getEulerApys.ts';

const abi = parseAbi(['function convertToAssets(uint shares) external view returns (uint)']);

export type EulerPricePool = EulerPool & {
  oracleId: string;
  decimals: string;
};

export const getEulerPrices = async (chainId: ChainId, pools: EulerPricePool[], tokenPrices: PricesById) => {
