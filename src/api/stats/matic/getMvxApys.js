...
const pools = require('../../../data/matic/mvxPools.json');

const DECIMALS = '1e18';
const SECONDS_PER_YEAR = 31536000;

const getMvxApys = async () => {
  let promises = [];
  let farmAprs = [];
...
const getTotalStakedInUsd = async pool => {
  let staked = 0;
  if (pool.name == 'mvx-mvlp') {
    const strategy = fetchContract(pool.strategy, StrategyABI, POLYGON_CHAIN_ID);
