...
const { POLYGON_CHAIN_ID: chainId, BASE_HPY } = require('../../../constants');
...
const getSolaceApy = async () => {
  ...
  const beefyPerformanceFee = getTotalPerformanceFeeForVault('solace-polygon-solace');
  const shareAfterBeefyPerformanceFee = 1 - beefyPerformanceFee;
  const apy = compound(apr, BASE_HPY, 1, shareAfterBeefyPerformanceFee);

  return {
    apys: {
      'solace-polygon-solace': apy,
    },
    apyBreakdowns: {
      'solace-polygon-solace': {
        vaultApr: apr,
