...
import pools from '../../../data/matic/mvxPools.json';
import { POLYGON_CHAIN_ID } from '../../../constants';
import { fetchContract } from '../../rpc/client';
import ERC20Abi from '../../../abis/ERC20Abi';
import MvxManagerAbi from '../../../abis/matic/MvxManager';

const getMvxPrices = async tokenPrices => {
  let prices = {};
  let promises = [];
...
const getLpTokenBalances = async pool => {
  const balanceCalls = pool.tokens.map(token => {
    const contract = fetchContract(token.address, ERC20Abi, POLYGON_CHAIN_ID);
...
const getLpPrice = async pool => {
  const glpManagerContract = fetchContract(pool.glpManager, MvxManagerAbi, POLYGON_CHAIN_ID);
  const glpContract = fetchContract(pool.address, ERC20Abi, POLYGON_CHAIN_ID);

  const results = await Promise.all([
    glpManagerContract.read.getAumInUsdm([true]),
